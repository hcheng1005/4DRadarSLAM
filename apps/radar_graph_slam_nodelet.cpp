  ...
  private:
    ...
    void cloud_callback(const nav_msgs::OdometryConstPtr &odom_msg, const sensor_msgs::PointCloud2::ConstPtr &cloud_msg)
    {
      ...
      if (pow(map2base_trans.transform.rotation.w, 2) + pow(map2base_trans.transform.rotation.x, 2) +
              pow(map2base_trans.transform.rotation.y, 2) + pow(map2base_trans.transform.rotation.z, 2) <
          pow(0.9, 2))
      {
      ...
      if (enable_preintegration)
      {
        ...
        odom_frame2frame.header.seq = sequ;
        sequ++;
    ...
    void imu_callback(const sensor_msgs::ImuConstPtr &imu_odom_msg)
    {
      ...
      if (cnt == 0)
      {
        ...
        Eigen::Matrix3d imu_mat_final;
        imu_mat_final = yawAngle * pitchAngle * rollAngle;
        ...
        std::cout << initial_pose(0, 0) << ", " << initial_pose(0, 1) << ", " << initial_pose(0, 2) << ", " << initial_pose(0, 3) << ", " << std::endl
                  ...
                  << initial_pose(3, 0) << ", " << initial_pose(3, 1) << ", " << initial_pose(3, 2) << ", " << initial_pose(3, 3) << ", " << std::endl
                  << std::endl;
        cnt = 1;
      }
    }
    ...
    geometry_msgs::Transform preIntegrationTransform()
    {
      ...
      if (!imu_odom_queue.empty())
      {
        ...
        if (use_egovel_preinteg_trans)
        {
          ...
          {
            q_via_x(i, 0) = twist_queue.at(i)->twist.linear.x;
            q_via_x(i, 1) = 0;
            q_via_x(i, 2) = 0;
            ...
            q_via_y(i, 0) = twist_queue.at(i)->twist.linear.y;
            q_via_y(i, 1) = 0;
            q_via_y(i, 2) = 0;
            t_via_y(i) = t_via_x(i);
            q_via_z(i, 0) = twist_queue.at(i)->twist.linear.z;
            q_via_z(i, 1) = 0;
            q_via_z(i, 2) = 0;
            t_via_z(i) = t_via_x(i);
          }
        ...
        if (use_egovel_preinteg_trans)
        { // Use Ego vel as translation integration
          for (size_t i = 0; i < imu_odom_end_index + 1; i++)
          {
            ...
            Eigen::Vector3d translation_ = Eigen::Vector3d(egovelIntegratorX->getPosition(imuOdomTime),
                                                           ...
                                                           egovelIntegratorZ->getPosition(imuOdomTime)) *
                                           dt;
        ...
        { // Use IMU as translation integration
    ...
    bool flush_barometer_queue()
    {
      ...
      auto remove_loc = std::upper_bound(barometer_queue.begin(), barometer_queue.end(), keyframes.back()->stamp, [=](const ros::Time &stamp, const barometer_bmp388::BarometerConstPtr &baropoint)
                                         { return stamp < baropoint->header.stamp; });
    ...
    void optimization_timer_callback(const ros::WallTimerEvent &event)
    {
      ...
      if (odom2base_pub.getNumSubscribers())
    ...
    visualization_msgs::MarkerArray create_marker_array(const ros::Time &stamp) const
    {
      ...
      loop_marker.color.r = 0.9;
      loop_marker.color.g = 0.9;
    ...
    bool flush_gps_queue()
    {
      ...
      {
        if (keyframe->index - last_gps_edge_index < gps_edge_intervals)
          continue;
      ...
      auto remove_loc = std::upper_bound(gps_navsat_queue.begin(), gps_navsat_queue.end(), keyframes.back()->stamp, [=](const ros::Time &stamp, const sensor_msgs::NavSatFix &geopoint)
