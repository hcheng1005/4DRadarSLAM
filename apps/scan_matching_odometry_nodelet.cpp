...
namespace radar_graph_slam
{
  ...
  private:
    ...
    void imu_callback(const sensor_msgs::ImuConstPtr &imu_msg)
    {
      ...
      imu->angular_velocity = imu_msg->angular_velocity;
      imu->linear_acceleration = imu_msg->linear_acceleration;
      ...
      imu->orientation.w = imu_quat_deskew.w();
      imu->orientation.x = imu_quat_deskew.x();
      imu->orientation.y = imu_quat_deskew.y();
      imu->orientation.z = imu_quat_deskew.z();
      {
    ...
    bool flush_imu_queue()
    {
      ...
      for (size_t i = 0; i < keyframes.size(); i++)
      {
        ...
        imu_.header = (*closest_imu)->header;
        imu_.orientation = (*closest_imu)->orientation;
        imu_.angular_velocity = (*closest_imu)->angular_velocity;
        imu_.linear_acceleration = (*closest_imu)->linear_acceleration;
      ...
      auto remove_loc = std::upper_bound(imu_queue.begin(), imu_queue.end(), keyframes.back()->stamp, [=](const ros::Time &stamp, const sensor_msgs::ImuConstPtr &imupoint)
                                         { return stamp < imupoint->header.stamp; });
    ...
    Eigen::Matrix4d matching(const ros::Time &stamp, const pcl::PointCloud<PointT>::ConstPtr &cloud)
    {
      ...
      // 这里是一个异常检测机制，前后帧计算出来的变换矩阵如果过大，则丢弃本次结果
      ...
      if (enable_transform_thresholding) // true
      {
        ...
        if (enable_imu_thresholding) // false
        {
          ...
          if (dx > max_diff_trans || da > max_diff_angle || too_large_trans)
          {
            ...
            if (too_large_trans)
              cout << "Too large transform! " << dx_rd << "[m] " << da_rd << "[deg] ";
        ...
        else
        {
          if (too_large_trans)
          {
            ...
            if (enable_scan_to_map)
            {
              ...
              odom_s2s_now = keyframe_pose_s2s * prev_trans_s2s * radar_delta;
          }
        }
      ...
      if (keyframe_updater->decide(Eigen::Isometry3d(odom_s2s_now), stamp))
      {
        ...
        {
          if (enable_scan_to_map)
            transformUpdate(odom_s2m_now);
          else
            transformUpdate(odom_s2s_now);
        }
