...
namespace radar_graph_slam
{
                                                            ...
                                                            const KeyFrame::Ptr &new_keyframe)
    {
        ...
        if (registration->hasConverged() == false || registration->getFitnessScore() > historyKeyframeFitnessScore)
        {
            ...
            std::cout << "\033[31m"
                      << "ICP fitness test failed. ICP convergence: " << converged << " (" << registration->getFitnessScore() << " > " << historyKeyframeFitnessScore << "). Reject this SC loop."
                      << "\033[0m" << std::endl;
            return nullptr;
        }
        ...
        std::cout << "\033[32m"
                  << " Add this ScanContext loop. ICP fitness test passed ( Fitness Score " << registration->getFitnessScore() << " < " << historyKeyframeFitnessScore << "). "
                  << "\033[0m" << std::endl;
    ...
    cv::Mat LoopDetector::makeSCImage(Eigen::MatrixXd src_mat)
    {
        ...
        for (int i = 0; i < scManager->PC_NUM_SECTOR; i++)
        { // 60
            for (int j = 0; j < scManager->PC_NUM_RING; j++)
            { // 20
